...
使えるようになるまでの全体像（下の手順0〜3に対応）:

- 手順0: PCに道具を2つ入れる（Node.js と Git）
- 手順1: Cloudflareの無料アカウントを作る
- 手順2: この設計図を手元にコピーし、コマンドを順番に貼り付けて実行する
- 手順3: 最後に表示される自分専用URLをスマホ・PCで開いて設定する
...
- **受け渡し**: iPhoneの共有シートから送信（iOSショートカット）、PCはCtrl+V（MacはCmd+V）で貼り付け（テキスト・URL・画像）
...
### 招待リンク（トークンの手入力なしで端末を追加する）

`https://あなた専用のURL/#setup=トークン`（トークンはURLエンコードする）を端末で開くと、トークンが自動で設定されます。設定画面での手入力と同じ扱いで、以降は普通に同期します。

- リンクを知っている人は全メモを読み書きできます。**招待リンクはパスワードと同じ扱いで**、渡す相手・渡し方（AirDropや1対1のメッセージ等）に注意してください
- トークンはURLのフラグメント（#以降）なのでサーバーやアクセスログには送られず、適用後はアドレスバーからも消えます
- 設定済みの端末で別のトークンのリンクを開いた場合は、上書きしてよいか確認ダイアログが出ます
- iPhoneは「ホーム画面に追加」で保存領域が切り替わるためトークンが引き継がれないことがあります。その場合はSafariでもう一度招待リンクを開くか、設定画面に手入力してください
...
> ZIPでダウンロードした場合: 最初の `git clone` の行は飛ばし、展開してできたフォルダ（`tanimemo-main`）に対して `cd tanimemo-main/app` から始めてください。
...
3. スマホは共有シートの「ホーム画面に追加」でアプリとしてインストールする
...
デプロイは「app をビルド → worker で `npx wrangler deploy`」の順です。D1のスキーマを変えたときは先に `npx wrangler d1 migrations apply tanimemo --remote` を実行します。トークンを変えるときは `npx wrangler secret put API_TOKEN` を実行します（変更後は各端末の設定画面とショートカットも更新します）。
